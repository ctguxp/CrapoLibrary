...
namespace System
  ...
  namespace Drawing
    ...
    Color KnownColors::FromKnownColor(KnownColor kc)
      {
      short num = (short)kc;
      Color result;
      if (num <= 0 || (int)num >= 175)
        {
        result = Color::FromArgb(0, 0, 0, 0);
        result.State((Color::ColorType)((intptr)result.State() | 4));
        }
      else
        {
        result.State((Color::ColorType)((intptr)Color::ColorType::ARGB | (intptr)Color::ColorType::Known | (intptr)Color::ColorType::Named));
        if (num < 27 || num > 169)
          {
          result.State((Color::ColorType)((intptr)result.State() | 8));
          }
        result.Value(((ulong)ArgbValues[(int)num]));
        }
